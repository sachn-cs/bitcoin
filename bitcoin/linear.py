...
def parse_signature_scalar(value: str, label: str) -> int:
    ...
    if not isinstance(value, str):
        raise InvalidLinearCoefficientError(f"{label} must be a hexadecimal string.")
    ...
    try:
        ...
        raise InvalidLinearCoefficientError(f"{label} is not valid hex.") from error
    return parsed
...
class LinearCoefficientRecord:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.alpha, int) or not 0 <= self.alpha < SECP256K1_ORDER:
            raise InvalidLinearCoefficientError("alpha must be reduced modulo n.")
        if not isinstance(self.beta, int) or not 0 <= self.beta < SECP256K1_ORDER:
            raise InvalidLinearCoefficientError("beta must be reduced modulo n.")
        if (
            not isinstance(self.sighash_flag, int)
            or not 0 <= self.sighash_flag <= 0xFFFFFFFF
        ):
            raise InvalidLinearCoefficientError("sighash_flag must be non-negative.")
        if not isinstance(self.script_type, str) or not self.script_type.strip():
            raise InvalidLinearCoefficientError(
