...
def sighash_segwit(tx: Tx, input_index: int, script: bytes, value: int, flag: int) -> bytes:
    """Compute the BIP-143 SegWit v0 sighash.

    Arguments:
        tx: The transaction.
        input_index: Index of the input being signed.
        script: The script code (redeemScript for P2SH-P2WPKH, witnessScript for P2WSH).
    ...
    # Hash sequences
    if flag & SIGHASH_ANYONECANPAY or (flag & SIGHASH_MASK) in (SIGHASH_NONE, SIGHASH_SINGLE):
        data.extend(b"\x00" * 32)
    else:
        hash_sequence = hash256(
            b"".join(txin.sequence.to_bytes(4, "little") for txin in tx.inputs)
        )
    ...
    if (flag & SIGHASH_MASK) == SIGHASH_NONE:
        ...
        hash_outputs = hash256(
            b"".join(
                out.value.to_bytes(8, "little") + encode_varint(len(out.script_pubkey)) + out.script_pubkey
